...
- *the one that unlocks a whole new level of possibilities* (TzufRougelike (HK): change `RemoveSpellUpgrades` to `false`) [hint by Joy the Bard!]
- *Robin Hood* (CrystalOOT: Bow) [hint by Deathclaw99!]
- *David v Goliath* (CrystalOOT: Slingshot) [hint by Deathclaw99!]
...
- *everyone's favourite oven mitt* (Caesius (OOT): "Goron Tunic") [hint by DiStegro!]
...
- *the one that should not be used in crowded spaces* (Jake13Snake (DOOM 1993): "Rocket launcher") [hint by Joy the Bard!]
- *Big Kaboom*  (superfid (Secret of Evermore): "Bazooka") [hint by Deathclaw99!]
...
- *communal amity* (Caesius (OOT): "Piece of Heart") [hint by DiStegro!]
- *the love of a rascal* (Caesius (OOT): "Heart Container") [hint by DiStegro!]
- *better printers* (Caesius (TS): "Max HP") [hint by DiStegro!]
- *Veggie Smoothie* (superfid (TS): increase `hp_cap` lower bound) [hint by Deathclaw99!]
- *Life Crystal* (CrystalOOT: Heart Container) [hint by Deathclaw99!]
...
- *a witch's brew.* (Caesius (Emerald): "Super Potion") [hint by DiStegro!]
- *Trashman* (SansEmeraldRL: "Leftovers") [hint by Deathclaw99!]
...
- *a better trainer* (Caesius (MC): reduce `combat_difficulty` by one level) [hint by DiStegro!]
- *Mr. Softener* (superfid (Secret of Evermore): change `difficulty` weights to exclude `hard`) [hint by Deathclaw99!]
...
- *Stronger Fuel* (JoyTheBard (OW): Boost Duration Upgrade) [hint by Deathclaw99!]
- *Efficient Fuel* (JoyTheBard (OW): Fuel Capacity Upgrade) [hint by Deathclaw99!]
...
- *It's time to get simpler* (superfid (C64): change `logic_difficulty` to `standard`) [hint by Deathclaw99!]
- *noclip_1* (superfid (Secret of Evermore): change `out_of_bounds` to `on`) [hint by Deathclaw99!]
- *BLJ = True*(superfid (Secret of Evermore): change `sequence_breaks` to `on`) [hint by Deathclaw99!]
...
- *maybe surpassing your dad is all you need* (MiroRogue (Emerald): remove weights of difficult `goal` settings) [hint by Joy the Bard!]
...
- *less management* (Caesius (MC): remove a boss from `required_bosses`) [hint by DiStegro!]
- *Dream Charm* (superfid (TS): limit `boss_rando` to only `off` or `scaled`) [hint by Deathclaw99!]
- *maybe this will tip the scales* (Caesius (TS): change `boss_rando` to `scaled`) [hint by DiStegro!]
- *Sad boy :(* (superfid (Secret of Evermore): change `short_boss_rush` to `true`) [hint by Deathclaw99!]
...
- *Let me in. LET ME IN!!!* (RRRRRRogue772: reduce `door_cost`) [hint by ChromaNyan!]
...
- *Calcium Break* (Brynatron (Terraria): add `Lucky Break` to `exclude_locations`) [hint by Deathclaw99!]
- *Housing Market Crash* (Brynatron (Terraria): add `Real Estate Agent` to `exclude_locations`) [hint by Deathclaw99!]
- *Too Squishy* (Brynatron (Terraria): add `The Great Slime Mitosis` to `exclude_locations`) [hint by Deathclaw99!]
- *Spell: Detect Trap* (Brynatron (Terraria): add `Watch Your Step!` to `exclude_locations`) [hint by Deathclaw99!]

## entrance rando
- *not in their natural environment but doin' just fine* (Roguinater (MC): set `shuffle_structures` to `true`) [hint by Joy the Bard!]
- *Sober Stellarium* (JoyTheBard (OW): change `randomize_orbits` to `false`) [hint by Deathclaw99!]
- *North to South* (JoyTheBard (OW): change `randomize_rotations` to `false`) [hint by Deathclaw99!]
...
- *i mean if you wanna make it easier to wake the big scary thing be my guest* (ChromaNyan (MC): reduce `egg_shards_required` by varied amounts) [hint by Joy the Bard!]
- *that's going to make a terrible pie.* (Sylsko (C64): reduce `total_strawberries` by 10) [hint by ChromaNyan!]
- *springwell garden city life* (Caesius (Marble Blast): change `goal` to `'30 levels'`) [hint by DiStegro!]
- *How many fruits am I missing again?* (superfid (C64): change `total_strawberries` to `random`) [hint by Deathclaw99!]
- *The goal comes closer* (superfid (C64): lower `strawberries_required_percentage`) [hint by Deathclaw99!]
...
- *sometimes you just need some luck* (Caesius (TS): make `damage_rando` better) [hint by DiStegro!]
...
- *Totem of Undying* (MiroRogue (Emerald): "Max Revive") [hint by ChromaNyan!]
- *a second chance.* (Caesius (Emerald): "Revive") [hint by DiStegro!]
- *gifts to the gods.* (Caesius (Emerald): "Sacred Ash") [hint by DiStegro!]
- *Chugjug* (CrystalOOT: Bottle) [hint by Deathclaw99!]
...
- *Looped Knowledge* (JoyTheBard (OW): increase `progression_balancing` by 25) [hint by Deathclaw99!]
...
- *no one likes making the walk of shame.* (JoyTheBard (DLCQ): "Checkpoint Pack") [hint by ChromaNyan!]
...
- *treats without a trick.* (Caesius (Emerald): "Rare Candy") [hint by DiStegro!]
- *Nebula Accessory* (superfid (TS): "Galaxy Earrings") [hint by Deathclaw99!]
- *some large jewellery* (Caesius (TS): "Galaxy Earrings") [hint by DiStegro!]
- *Faster Rounds* (superfid (TS): increase `extra_earrings_xp` lower bound) [hint by Deathclaw99!]
...
- *It's dangerous to go alone.* (superfid (LTTP): change `swordless` to `false`) [hint by ChromaNyan!]
- *a better foil* (Caesius (OOT): "Biggoron Sword") [hint by DiStegro!]
...
- *Hospitable Home* (JoyTheBard (OW): make `spawn` more reasonable) [hint by Deathclaw99!]
...
- *so cold...* (superfid (Secret of Evermore): "30 Cryo Blast") [hint by Deathclaw99!]
...
- *no need to push your problems onto others.* (MiroRogue (MC): change `send_defeated_mobs` to `false`) [hint by ChromaNyan!]
- *You feel more lonely* (superfid (C64): change `badeline_chaser_frequency` to be better) [hint by Deathclaw99!]
...
- *anarchy in the streets* (Caesius (MC): "Looting III Book") [hint by DiStegro!]

## travel
- *come home!* (Sylsko (TUNC): Dath Stone)
- *Escape Rope* (CrystalOOT: Farore's Wind) [hint by Deathclaw99!]
- *Instant Transmission* (Brynatron (Terraria): Reward: Rod of Discord) [hint by Deathclaw99!]
- *Zoom* (SansEmeraldRL: "HM02 Fly") [hint by Deathclaw99!]
...
- *a chance to try again* (Caesius (Marble Blast): "Time Travel (-10s)") [hint by DiStegro!]
- *ctrl-z* (Caesius (Marble Blast): "Time Travel (-5s)") [hint by DiStegro!]
- *a cultural shift away from windows* (Caesius (TS): "Max Sand") [hint by DiStegro!]

## ammo
- *take a shot!* (Roguinater (Peggle Nights): set `shuffle_starting_balls` to `false`)
- *FULLCLIP* (superfid (Secret of Evermore): change `fix_infinite_ammo` to `false`) [hint by Deathclaw99!]
...
- *the one that lets you make more with less* (sheen (Factorio): reduce `recipe_ingredients_offset` by 1) [hint by Joy the Bard!]
- *wait, you didn't want to go in order?* (jf-rogue (TS): change `Inverted: true` weight to 0) [hint by Joy the Bard!]
- *there's something more likeable about you. did you get a haircut?* (Brynatron (SDV): change `friendship_multiplier` to `random-range-middle-100-400`) [hint by Joy the Bard!]
...
- *All the Bling* (superfid (TS): change `start_with_jewelry_box` to `true`) [hint by Deathclaw99!]
- *Bruce Lee* (Brynatron (Terraria): Reward: Black Belt) [hint by Deathclaw99!]
- *Fire Resistance 4* (Brynatron (Terraria): Reward: Lava Charm) [hint by Deathclaw99!]
- *Hungry Larvitar* (SansEmeraldRL: change `extra_boulders` to `false`) [hint by Deathclaw99!]
- *Finally, Earplugs* (SansEmeraldRL: change `move_blacklist` to `["Perish Song"]`) [hint by Deathclaw99!]
